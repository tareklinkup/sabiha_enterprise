...
class Account extends CI_Controller {
    ...
    function getOtherIncomeExpense(){
        ...
        if(isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != ''){
            ...
            $repairClause = " and s.date between '$data->dateFrom' and '$data->dateTo'";
        }
    ...
    public function getCashLedger() {
        ...
        $ledger = $this->db->query("
            ...
            select 
                sm.id ,
                sm.date,
                concat('Repair - ', sm.invoice, ' - ', c.Customer_Name, ' (', c.Customer_Code, ')', ' - Bill: ', sm.total) as description,
                sm.paid as in_amount,
                0.00 as out_amount
            from  tbl_servicemaster sm 
            join tbl_customer c on c.Customer_SlNo = sm.customer_id
            where sm.status = 'a'
            and sm.branch_id = '$this->brunch'
            and sm.date between '$data->fromDate' and '$data->toDate'
