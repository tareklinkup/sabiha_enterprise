...
class Page extends CI_Controller {
    ...
    public function catdelete(){
        ...
        // check product entry
        $checkProduct = $this->db->query("select * from tbl_product where ProductCategory_ID = ? and status = 'a'", $id);
        if($checkProduct->num_rows() > 0) {
            $success = 'Your could not delete this category !. Please product delete first';
            echo json_encode($success);
            exit;
        }
    ...
    public function subCategory() {
        ...
        $data['content'] = $this->load->view('Administrator/subcategory', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addSubcategory() {
        ...
        try {
            ...
            $checkDuplicate = $this->db->query("
                ...
                and name = ? 
                and branch_id = ?
                ", [$data->category_id, $data->name, $this->session->userdata('BRANCHid')]);
            
            if($checkDuplicate->num_rows() > 0) {
                $res->success = false;
                $res->message = "Already category name exisits";
    ...
    public function updateSubcategory() {
        ...
        try {
            ...
            $checkDuplicate = $this->db->query("
                ...
                and branch_id = ?
                and id != ?
                ", [$data->category_id, $data->name, $this->session->userdata('BRANCHid'), $data->id]);
    ...
    public function deleteSubcategory() {
        ...
        try {
            ...
            // check product entry
            $checkProduct = $this->db->query("select * from tbl_product where subcategory_id = ? and status = 'a'", $data->id);
            if($checkProduct->num_rows() > 0) {
                $res->message = 'Your could not delete this subcategory !. Please product delete first';
                $res->success = false;
                echo json_encode($res);
                exit;
            }

            $this->db->set('status', 'd')->where('id', $data->id)->update('tbl_subcategory');

            $res->message = 'Deleted successfully';
            $res->success = true;
        } catch (\Exception $e) {
            $res->success = false;
            $res->message = 'faliled '. $e->getMessage();
        }
