        ...
        <div class="col-md-8">
            <form class="form-horizontal" @submit.prevent="addDamage">
                ...
                <div class="form-group" style="display: none;" :style="{display: selectedProduct.is_serial == true ? '' : 'none'}">
                    ...
                    <label class="col-sm-1 control-label no-padding-right">:</label>
                    <div class="col-sm-5">
                        <v-select v-bind:options="serials" label="ps_serial_number" v-model="serial" v-on:input="serialOnChange"></v-select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <label class="col-sm-1 control-label no-padding-right">:</label>
                    <div class="col-sm-5">
                        <input type="number" placeholder="Quantity" class="form-control" v-model="damage.DamageDetails_DamageQuantity" required v-on:input="calculateTotal" :readonly="serial.ps_id != ''? true : false" />
                    </div>
                </div>
    ...
    new Vue({
        ...
        methods: {
            ...
            async editDamage(damage) {
                ...
                //     is_serial: damage.is_serial
                // }
                this.selectedProduct = this.products.find(p => p.Product_SlNo == damage.Product_SlNo)
                setTimeout(() => {
                    if (damage.is_serial == true) {
                        this.serial = {
                            ps_id: damage.damageSerial.ps_id,
                            ps_serial_number: damage.damageSerial.ps_serial_number
                        }
