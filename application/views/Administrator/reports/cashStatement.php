		...
		<div class="row" style="padding-top:10px;">
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody style="display:none;" v-bind:style="{display: services.length > 0 ? '' : 'none'}">
						<tr v-for="(data, ind) in services">
							<td>{{ ind + 1 }}</td>
							<td>{{ data.date }}</td>
							<td>{{ data.invoice }}</td>
							<td style="text-align: right;">{{ data.paid | decimal }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="3" style="text-align:right">Total</td>
							<td style="text-align:right;">
								<span v-if="services.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: services.length > 0 ? '' : 'none'}">{{ totalServiceAmount | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalServiceAmount() {
				return this.services.reduce((p, c) => { return +p + +c.paid}, 0).toFixed(2)
			},
			totalCashIn(){
				return parseFloat(this.totalSales) + 
					...
					parseFloat(this.totalServiceAmount);
			},
			totalCashOut(){
