						...
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="services.length > 0">
									<tr>
										<td class="sub-heading">Repair</td>
										<td class="sub-value">{{ totalService }}</td>
									</tr>
									<tr v-for="service in services">
										<td>{{ service.Customer_Name }}</td>
										<td>{{ service.totalAmount | decimal }}</td>
									</tr>
								</template>
...
<script>
	new Vue({
		...
		methods: {
			...
			getServices(){
				axios.post('/get_service', this.filter)
				.then(res => { 
					let services = res.data.services;
					services = _.groupBy(services, 'customer_id');
					services = _.toArray(services);
					services = services.map(service => {
						service[0].totalAmount = service.reduce((p, c) => { return p + parseFloat(c.paid) }, 0);
						return service[0];
					})
