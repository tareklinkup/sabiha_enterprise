...
<script>
	...
	new Vue({
		...
		computed:{
			...
			serviceCharge(){
				return this.reportData.reduce((prev, cur) => { return prev + parseFloat(cur.service_charge) }, 0).toFixed(2);
			},
			otherIncome(){
				return (parseFloat(this.totalTransportCost) + parseFloat(this.otherIncomeExpense.income) + parseFloat(this.serviceCharge)).toFixed(2);
			}
		},
		methods: {
			getCustomers(){
