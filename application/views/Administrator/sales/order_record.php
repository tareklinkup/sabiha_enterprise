						...
						<template v-for="sale in sales">
							<tr>
								...
								<td style="text-align:center;">
									<a href="" title="Order Invoice" v-bind:href="`/sale_invoice_print/${sale.SaleMaster_SlNo}`" target="_blank"><i class="fa fa-file"></i></a>
                                    <a href="" title="Approved Order" @click.prevent="approveSale(sale.SaleMaster_SlNo)"><i class="fa fa-check"></i></a>
                                    <?php if($this->session->userdata('accountType') != 'u'){?>
										<a href="" title="Edit Order" v-bind:href="`/order/${sale.SaleMaster_SlNo}`"><i class="fa fa-edit"></i></a>
										<a href="" title="Delete Order" @click.prevent="deleteOrder(sale.SaleMaster_SlNo)"><i class="fa fa-trash"></i></a>
									<?php }?>
								</td>
							</tr>
				...
				<table 
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:right;">{{ sale.service_charge }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.service_charge)}, 0).toFixed(2) }}</td>
...
<script>
	...
	new Vue({
		...
		methods: {
            ...
            approveSale(saleId){
				let approveConf = confirm('Are you sure?');
				if(approveConf == false){
					return;
				}
				axios.post('/approve_sales', {saleId: saleId})
				.then(res => {
					let r = res.data;
					alert(r.message);
					if(r.success){
