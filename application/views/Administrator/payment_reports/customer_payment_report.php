			...
			<table class="table table-bordered">
				...
				<tfoot>
					<tr style="font-weight: 600;">
						...
						<td style="text-align: right;">{{ (payments.reduce((prev,curr)=>{return +prev + +curr.bill},0)).toFixed(2) }}</td>
						<td style="text-align: right;">{{ (payments.reduce((prev,curr)=>{return +prev + +curr.paid},0)).toFixed(2) }}</td>
						<td style="text-align: right;">{{ (payments.reduce((prev,curr)=>{return +prev + +curr.due},0)).toFixed(2) }}</td>
						<td style="text-align: right;">{{ (payments.reduce((prev,curr)=>{return +prev + +curr.returned},0)).toFixed(2) }}</td>
						<td style="text-align: right;">{{ (payments.reduce((prev,curr)=>{return +prev + +curr.paid_out},0)).toFixed(2) }}</td>
						<td></td>
					</tr>
				</tfoot>
			</table>
		...
		methods: {
			getCustomers() {
				axios.get('/get_customers').then(res => {
					this.customers = res.data.filter(item => item.is_supplier == false);
				})
			},
			getReport() {
