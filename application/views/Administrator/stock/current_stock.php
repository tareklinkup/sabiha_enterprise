...
<div id="stock">
	...
	<div class="row">
		<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
			...
			<div class="form-group" style="margin-top:10px;" v-if="selectedSearchType.value == 'category'">
				<div class="col-sm-2" style="margin-left:15px;">
					<v-select v-bind:options="filterCategory" v-model="selectedSubcategory" label="name"></v-select>
				</div>
			</div>
				...
				<table class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td v-if="product.is_serial == true">
								<button type="button" class="button edit" @click="viewSerial(product.product_id)">
				...
				<table class="table table-bordered" v-if="searchType != 'current' && searchType != null" style="display:none;" v-bind:style="{display: searchType != 'current' && searchType != null ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td>{{ product.Product_Name }}</td>
							<td>{{ product.ProductCategory_Name }}</td>
							<td>{{ product.productSubCategory_name }}</td>
							...
							<td>{{ product.stock_value | decimal }}</td>
							<td v-if="product.is_serial == true">
								<button type="button" class="button edit" @click="viewSerial(product.Product_SlNo)"><i class="fa fa-list"></i></button>
							</td>
							<td></td>
						</tr>
					</tbody>
	...
	new Vue({
		...
		methods: {
			...
			getStock() {
				...
				if (this.searchType == 'category' && this.selectedSubcategory != null) {
					parameters.subCategoryId = this.selectedSubcategory.id;
				}
