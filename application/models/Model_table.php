...
class Model_Table extends CI_Model
{
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                select 
                    ifnull(sum(s.paid), 0)
                from tbl_servicemaster s
                where s.status = 'a'
                and s.branch_id = " . $this->session->userdata('BRANCHid') . "
                " . ($date == null ? "" : " and s.date < '$date'") . "
            ) as repair_amount, 
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(sm.return_amount), 0.00)
                from tbl_salesmaster sm
                where sm.SalseCustomer_IDNo = c.Customer_SlNo
                " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
                and sm.SalseCustomer_IDNo = c.Customer_SlNo
                and sm.Status = 'a') as posReturnAmount,
            ...
            (select (billAmount + paidOutAmount + posReturnAmount) - (paidAmount + returnedAmount)) as dueAmount
